...
public class AMBannerAdView: AMAdView {
    ...
    private var confViewURLs: [String]?
        ...
        if response.successful {
            ...
            if (adObject is UIView) {
                ...
                if adResponseInfo?.adType == .banner { //&& !(adObjectHandler is AMNativeStandardAdResponse) {
                    ...
                    confViewURLs = []
                    if let i = adObjectHandler.impressionUrls{
                        for url in i{
                            if url.contains("cet=4"){
...
// MARK: - UIView observer methods.
    public override func didMoveToWindow() {
        ...
        if self.window == nil {
            universalAdFetcher?.stopAutoRefreshTimer()
        } else {
            universalAdFetcher?.setupAutoRefreshTimer()
