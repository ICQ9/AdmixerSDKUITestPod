...
// MARK: - WKScriptMessageHandler.
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if (eventName == "adReady") {
            ...
            if loadingDelegate?.responds(to: #selector(AMUniversalAdFetcher.didCompleteFirstLoadWithError(from:))) ?? false {
                let lockQueue = DispatchQueue(label: "self")
                lockQueue.sync {
                    loadingDelegate?.didCompleteFirstLoadWithError(from: self)
                }
            }
...
// MARK: - MRAID
    func processWebViewDidFinishLoad() {
        if !completedFirstLoad {
            ...
            fixAdmixerCreativeClickFunction()
        }
    }
    ...
    func fixAdmixerCreativeClickFunction() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            ...
            function __findIframe() {
                var admixDiv = document.querySelectorAll('div[id^="admixdiv"]');
                if (admixDiv && admixDiv.length > 0) {
                    var fms = document.getElementsByTagName('iframe');
                    var wasFound = false;
                    for (var i = 0; i < fms.length; i++) {
                        if (!fms[i].src && fms[i].contentDocument && fms[i].contentDocument.readyState == 'complete') {
                            var container = fms[i].contentDocument.getElementById('container');
                            if (container) {
                                container.onclick = (function(_window) {
                                    return function(_event) {
                                        prevent(_event);
                                        _window.globalHTML5Api.click();
                                        _window.globalHTML5Api.enableScroll();
                                    }
                                })(fms[i].contentWindow);
                                wasFound = true
                            }
...
@objc protocol AMAdWebViewControllerLoadingDelegate: NSObjectProtocol {
    ...
    func didCompleteFirstLoadWithError(from controller: AMAdWebViewController?)
