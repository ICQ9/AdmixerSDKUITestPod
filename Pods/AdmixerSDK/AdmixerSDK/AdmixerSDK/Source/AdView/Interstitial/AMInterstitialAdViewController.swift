//
//  AMInterstitialAdViewController.swift
//  AdmixerSDK
//
...
class AMInterstitialAdViewController: UIViewController {
    weak var delegate: AMInterstitialAdViewControllerDelegate?
    ...
    init() {
        let nibName = AMPathForAMResource("AMInterstitialAdViewController", "nib") ?? ""
        if nibName.isEmpty {
            AMLogError("Could not instantiate interstitial controller because of missing NIB file")
        }
        super.init(nibName: "AMInterstitialAdViewController", bundle: resourcesBundle)
    ...
    @objc func progressTimerDidFire(_ timer: Timer?) {
        ...
        if autoDismissAdDelay > -1 {
            progressView.progress = Float(((timeShown ?? 0.0) / autoDismissAdDelay))
            ...
            progressView.progress = Float(((timeShown ?? 0.0) / closeButtonDelay))
            if (timeShown ?? 0.0) >= closeButtonDelay && closeButton.isHidden == true {
                closeButton.isHidden = false
                closeButton.isUserInteractionEnabled = true
                stopCountdownTimer()
    ...
    func dismissAd(animated: Bool) {
        dismissing = true
        delegate?.interstitialAdViewControllerShouldDismiss(self, with: animated)
    }
...
@objc protocol AMInterstitialAdViewControllerDelegate: NSObjectProtocol {
    func interstitialAdViewControllerShouldDismiss(_ controller: AMInterstitialAdViewController?, with animate: Bool)
